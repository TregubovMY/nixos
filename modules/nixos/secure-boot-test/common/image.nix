# Vendored from lanzaboote's nix/tests/lanzaboote/common/image.nix (pinned
# rev 7c9a54a7f87b4539ddbd8bda09a8a5f5f9361aa9, matching this repo's
# flake.lock — see ../systemd-initrd.nix for the full provenance note).
#
# Same one-level path-depth adjustment as ./lanzaboote.nix: all
# `../../fixtures/uefi-keys` references below are `../fixtures/uefi-keys`
# here. Content is otherwise unchanged.
#
# This is upstream's own systemd-repart image-building machinery — a
# completely different mechanism from this repo's real disk layout
# (disko-luks-btrfs.nix uses disko to partition a blank virtual disk at VM
# boot time; this builds a pre-baked image with separate ESP/nix-store/root
# partitions ahead of time, UEFI auth-variables written straight onto the
# ESP for auto-enrollment via `sbctl enroll-keys`). See
# ../systemd-initrd.nix's header comment and docs/superpowers/specs/
# 2026-08-10-secure-boot-design.md ("Two Checks, Not One Combined Test")
# for why these two are deliberately not combined.
{
  config,
  lib,
  pkgs,
