...
# target machine. `nixos-rebuild build-vm` supplies its own disk/kernel
# boot path (nixos/modules/virtualisation/qemu-vm.nix), so no
# boot.loader/fileSystems config is needed here. Once the full hosts/
# restructuring (disko/LUKS/Secure Boot, see system-plan.md §3-4) exists,
# import dev-databases.nix into the real host's configuration.nix instead
