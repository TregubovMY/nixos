...
`agent-uv-tools` — общие (не тянуть по новой версии ruby/не переустанавливать
`uv tool install`-пакеты на каждый запуск), `agent-cache-*` — свой на проект
(индекс кодовой базы, история сессии claude-code/opencode).
...
- Общие volumes `agent-mise`, `agent-uv-tools` и `agent-npm-global` (см.
  ...
  агента", не целенаправленная атака, и переустанавливать рантайм/тулинг
  на каждый проект ради этого не стоит. Если модель угроз изменится —
...
### 9.8 Секреты и соседи (sidecar) для реальных запросов и системных тестов

Принцип: **секрет в безопасности, только если его не может прочитать ни
один процесс, код или конфиг которого меняет агент.** Правила вида «не
читай `.env`» в `.claude/settings.json` — дополнительный слой, не
граница. Граница — контейнер: всё, что видит процесс агента, агент может
прочитать, а открытая сеть (9.2) позволяет это вынести.

Таксономия (что куда класть):

| Секрет | Как даётся агенту | Остаточный риск |
|---|---|---|
| Дешёвый тестовый/sandbox-ключ | Можно прямо в контейнер проекта (белый список `bin/agent-sandbox`) | Агент видит ключ — приемлемо, раз ключ дешёвый |
| API-ключ/токен в заголовке, ценный | Egress-прокси с подстановкой: в контейнере заглушка, прокси снаружи подставляет настоящий ключ только для разрешённых хостов (следующий этап, кандидат — iron-proxy) | Использование в пределах разрешённого, без кражи |
| Ключ подписи / клиентский сертификат | **Сосед** (`bin/agent-sidecar`): отдельный контейнер держит ключ и делает только подпись/запрос | Агент может подписать что угодно, пока сосед поднят |
| Боевой ключ с правом записи/деньгами | Никак | — |

Как устроен сосед (`bin/agent-sidecar`, `bin/agent-secret-load`):

- **Секрет** — из Bitwarden через `rbw` (`agent-secret-load <имя>
  <элемент> [--field F | --notes] [--base64]`) в podman secret, по
  конвейеру без временных файлов и без argv. `rbw` не умеет вложения,
  поэтому бинарный секрет (например, криптоконтейнер) хранится в
  Bitwarden base64-строкой в поле и декодируется `--base64`.
  **Оговорка:** штатный драйвер podman secrets (`file`) сам хранит секрет
  в своём хранилище (`~/.local/share/containers/storage/secrets/
  filedriver`, права 0600, на LUKS-диске) до `agent-secret-load --rm`.
  Принято на этом этапе как хранилище podman, а не случайная копия;
  ужесточение без хранения на диске — `shell`-драйвер podman secrets,
  у которого `lookup` вызывает `rbw` в момент старта контейнера (не
  реализовано и не проверено).
- **Сосед** получает секрет tmpfs-файлом `/run/secrets/<имя>` (`podman
  run --secret`), работает с `--cap-drop=all` и `no-new-privileges`, в
  **внутренней** podman-сети проекта (`agent-sb-<hash>`, `--internal` —
  без маршрута наружу) под DNS-именем соседа. Контейнер агента
  подключается к этой сети автоматически при следующем старте и ходит к
  соседу по имени (`http://signer:8080`).
- **Образ соседа собирается только из коммита**: `agent-sidecar promote
  <проект> <имя> <commit>` делает `git archive <commit> <CONTEXT>` во
  временный каталог и `podman build` оттуда — незакоммиченные и
  неотслеживаемые файлы рабочего дерева (где работает агент) в образ не
  попадают. `promote` — это человеческое «я посмотрел дифф кода, который
  касается ключа, пусть работает рядом с ним». Каждое продвижение
  записывается в `<config-dir>/<имя>.promoted`. Пока `promote` не было,
  изменения сервиса агент тестирует на одноразовом тестовом ключе.
  Рекомендация для проектов: держать код, касающийся ключа, минимальным
  отдельным ядром («загрузить ключ + подписать байты»), чтобы ревью перед
  `promote` было коротким.
- **Конфиг соседей — на хосте**, вне смонтированного проекта:
  `~/.config/agent-sandbox/projects/<hash>/sidecars/<имя>.conf`
  (`agent-sidecar config-dir <проект>`), строки `CONTEXT=`,
  `CONTAINERFILE=`, `SECRETS=`. Файл разбирается как KEY=VALUE, никогда не
  исполняется. Если бы конфиг лежал в проекте, агент переписал бы его и
  расширил себе доступ.

Пример (подпись запросов в тестовую ЕСИА криптоконтейнером):

```bash
agent-secret-load esia-test-key "ESIA test signer" --field container-b64 --base64
mkdir -p "$(agent-sidecar config-dir ~/code/proj)"
printf 'CONTEXT=services/signer\nSECRETS=esia-test-key\n' \
  > "$(agent-sidecar config-dir ~/code/proj)/signer.conf"
agent-sidecar promote ~/code/proj signer <проверенный-commit>
agent-sidecar up ~/code/proj signer
agent-sandbox up ~/code/proj     # агент видит http://signer:<порт>, ключа — нет
```

Остаточные риски, принятые осознанно:

- **Злоупотребление без кражи**: пока сосед поднят, агент может подписать
  любые данные и сделать от имени ИС любые запросы в тестовый контур.
  Ограничивается тем, что ключ тестовый (держать под рукой процедуру
  перевыпуска и перерегистрации), журналом соседа и тем, что сосед
  поднимается только на время работы.
- **Ревью перед `promote` — человеческий шаг.** Если продвинуть коммит, в
  котором код соседа отдаёт ключ наружу (в ответе, в логах), изоляция
  этого не остановит — сеть у соседа закрыта, но канал «ответ агенту»
  остаётся.
- **Сборка образа соседа** выполняет `RUN`-шаги из коммита с обычной
  сетью podman build — но без секретов (секрет подключается только при
  `up`).
- **Не проверено вживую** (в среде разработки нет podman): одновременное
  подключение контейнера агента к сети `podman` и внутренней сети
  проекта (`--network podman --network agent-sb-<hash>`), DNS-алиасы во
  внутренней сети, `--secret` для rootless podman. Логика скриптов
  покрыта тестами с фейковыми `podman`/`rbw` (`tests/agent-sidecar-test.sh`,
  flake-check `agent-sandbox-cli`); ручной чек-лист — README.
- `agent-testrun` (прогон *непроверенного* кода с настоящим ключом без
  сети и с фильтром вывода) и egress-прокси — следующие этапы, по
  умолчанию выключены/не реализованы.
