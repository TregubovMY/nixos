...
- **Второй LUKS-контейнер не означает второй пароль при загрузке:**
  initrd-разблокировка NixOS автоматически пробует уже введённый
  пароль на следующих `boot.initrd.luks.devices` — при **одинаковой**
  парольной фразе у обоих контейнеров загрузка всё равно спрашивает
  пароль один раз. При реальной установке важно не перепутать и
  задать оба пароля одинаковыми (иначе загрузка будет спрашивать пароль
  дважды) — см. `README.md`.
