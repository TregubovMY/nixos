...
Переносимая NixOS-конфигурация (flake): disko-разметка диска + LUKS,
systemd-boot, Hyprland-десктоп, home-manager dotfiles, секреты через
sops-nix. Полное описание структуры и архитектуры — `system-plan.md`.
Правила работы агента с этим репозиторием — `CLAUDE.md`.

**Про эту ветку:** это рабочая ветка отдельного плана (перевод по
хоткею), собранная в изолированном git-worktree. Плоские
`flake.nix`/`configuration.nix`/`disko.nix`/`home.nix` здесь намеренно не
восстановлены — на момент создания ветки они уже считались вытесненными
модульной структурой из `system-plan.md` §3 (ещё не собранной, но это
уже отражено в рабочем дереве `master`, просто не закоммичено там). Реальный
disko + `nixos-install` цикл разметки диска описан в `master`'s
`README.md`/`disko.nix`, пока не собран `hosts/`-модуль.
